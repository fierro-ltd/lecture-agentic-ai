...
function defaultApiUrl() {
  // HAST API ports:
  //   - localhost dev: :8000 (FastAPI direct)
  //   - public demo via Caddy: :2096 (one of the Cloudflare-allowed HTTPS ports)
  // Paperclip lives on :8443 instead, but Paperclip is NOT the API the
  // demo UI talks to.
  var proto = window.location.protocol;
  var host  = window.location.hostname;
  var port  = proto === 'https:' ? '2096' : '8000';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Update API docs link to match configured API URL
  function updateApiLink() {
    var link = document.getElementById('linkApi');
    if (link) link.href = apiUrlInput.value.replace(/\/$/, '') + '/docs';
  }
  updateApiLink();
  apiUrlInput.addEventListener('input', updateApiLink);
